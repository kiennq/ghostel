...
const module_version = @import("src/version.zig");
...
pub fn build(b: *std.Build) void {
    ...
    const build_version = module_version.selectBuildVersion(
        b.graph.environ_map.get("GHOSTEL_VERSION"),
    );
    version_options.addOption(
        [:0]const u8,
        "version",
        b.allocator.dupeZ(u8, build_version) catch @panic("OOM"),
    );
    ...
    target_mod.addOptions("build_options", version_options);
    ...
    target_check_mod.addOptions("build_options", version_options);
    ...
    target_test_mod.addOptions("build_options", version_options);
