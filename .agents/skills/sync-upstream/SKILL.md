---
name: sync-upstream
description: Use when replaying or rebasing the Ghostel fork onto upstream/main, especially when upstream touches workflow cleanup, Zig/native build, dyn-loader, ConPTY, nonblocking native queues, renderer performance, title, cursor, or resize behavior.
---
...
4. dynamic loader support and the bundled runtime installer
5. Windows ConPTY via `emacs-util-mods`
6. non-mutex native backend command/event delivery
7. detached native cleanup and child reaping
...
customizations, and old conflict-resolution leftovers. Use the topics to audit
what must survive the sync, not to force extra commits that are not present on
`main`.

## Nonblocking Native Queue Invariant

The native backend architecture exists to keep Emacs' main thread responsive:

- `src/RingQueue.zig` is the fixed-capacity multi-producer/single-consumer queue.
- Emacs-facing producers enqueue writes, resizes, and stop commands, then signal
  the backend wake primitive; they do not take a backend handoff mutex or wait
  for the reader.
- The native reader thread is the sole queue consumer and backend owner.
- Native teardown detaches output and delegates reader joins, final draining,
  child waits, and memory destruction to cleanup/reaper threads.
- Queue-full and wake failures are reported explicitly; do not replace them
  with a blocking retry on the Emacs thread.

Never reintroduce `backend_handoff_mutex`, synchronous reader joins, child
...
7. After the full replay, diff the key files (`src/module.zig`, `src/Renderer.zig`, `src/NativeProcess.zig`, `src/RingQueue.zig`, `src/ConPtyProcess.zig`, `src/PosixPtyProcess.zig`, `lisp/ghostel.el`, `lisp/ghostel-module-install.el`, and their tests) between the old fork tip and the new replay tip. Any intentional fork addition that disappeared is a regression that must be restored before pushing.
...
- **Preserve allowlisted fork behavior, otherwise prefer upstream.** Conflict
  blocks containing dyn-loader ABI, ConPTY, nonblocking queue/lifecycle,
  renderer performance, title, cursor, or resize behavior must keep that
...
- **ghostel.el owns ConPTY coalescing and runtime helpers.** The fork adds `ghostel--conpty-active-p`, `conpty--read-pending`, `ghostel--coalesce-*`, and the loader bootstrap chain. It also owns `ghostel-ignore-cursor-change` and visible-window redraw deferral. These are intentional fork behavior.
...
- `ghostel.el` should use identity process dimensions for terminal creation, startup, and resize. Do not reintroduce the stale one-column width buffer (`window-max-chars-per-line - 1`).
- Terminal title behavior should stay enabled; preserve upstream title callbacks and `ghostel--set-title` wiring unless explicitly changed.
...
4. Common post-rebase failures:
   - **Tests expecting `(1- width)` or `(1- height)`** — the one-column width buffer is stale; update expected values to identity dimensions.
