...
test "tryLockTerm declines another thread's lock" {
    ...
    var acquired: std.Thread.Semaphore = .{};
    var release: std.Thread.Semaphore = .{};
    const Holder = struct {
        fn run(
            native_process: *Self,
            lock_acquired: *std.Thread.Semaphore,
            lock_release: *std.Thread.Semaphore,
        ) void {
            native_process.lockTerm();
            lock_acquired.post();
            lock_release.wait();
