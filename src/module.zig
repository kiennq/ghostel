...
const version: [:0]const u8 = "0.24.0";
...
return env.makeString(term.renderer.debugState(term, &buf));
}
...
const pos = term.renderer.cursorPosition(term) catch |err| {
env.signalErrorf("ghostel: cursor position failed: {s}", .{@errorName(err)});
return env.nil();
...
const offset = term.renderer.cursorRowCharOffset(term) catch |err| {
env.signalErrorf("ghostel: cursor row char offset failed: {s}", .{@errorName(err)});
return env.nil();
} orelse return env.nil();
...
const empty = term.renderer.cursorOnEmptyRow(term) catch |err| {
...
const env = emacs.Env.init(raw_env.?);
const term = env.getUserPtr(Terminal, args[0]) orelse return env.nil();
const line = term.renderer.redrawFullScrollback(env, term) catch |err| {
env.logStackTrace(@errorReturnTrace());
