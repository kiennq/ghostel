...
const ExportId = enum(u32) {
    ...
    write_pty = 38,
    spawn_native_process = 39,
    kill_native_process = 40,
    set_process_pid = 41,
    encode_paste = 42,
};
...
pub const ghostel_export_descriptors = [_]loader.ExportDescriptor{
    ...
    descriptor(.write_input, entryByName(GhostelTerm.emacs_functions, "ghostel--write-vt")),
    ...
    descriptor(.pty_password_input_p, entryByName(GhostelTerm.emacs_functions, "ghostel--pty-password-input-p")),
    ...
    descriptor(.write_pty, entryByName(GhostelTerm.emacs_functions, "ghostel--write-pty")),
    descriptor(.spawn_native_process, entryByName(GhostelTerm.emacs_functions, "ghostel--spawn-native-process")),
    descriptor(.kill_native_process, entryByName(GhostelTerm.emacs_functions, "ghostel--kill-native-process")),
    descriptor(.set_process_pid, entryByName(GhostelTerm.emacs_functions, "ghostel--set-process-pid")),
    descriptor(.encode_paste, entryByName(GhostelTerm.emacs_functions, "ghostel--encode-paste")),
};
...
pub fn invokeExport(
    ...
    const id_enum = std.enums.fromInt(ExportId, export_id) orelse {
    ...
    return switch (id_enum) {
        ...
        .write_input => callEntry(GhostelTerm.emacs_functions, "ghostel--write-vt", env, nargs, args),
        ...
        .pty_password_input_p => callEntry(GhostelTerm.emacs_functions, "ghostel--pty-password-input-p", env, nargs, args),
        ...
        .write_pty => callEntry(GhostelTerm.emacs_functions, "ghostel--write-pty", env, nargs, args),
        .spawn_native_process => callEntry(GhostelTerm.emacs_functions, "ghostel--spawn-native-process", env, nargs, args),
        .kill_native_process => callEntry(GhostelTerm.emacs_functions, "ghostel--kill-native-process", env, nargs, args),
        .set_process_pid => callEntry(GhostelTerm.emacs_functions, "ghostel--set-process-pid", env, nargs, args),
        .encode_paste => callEntry(GhostelTerm.emacs_functions, "ghostel--encode-paste", env, nargs, args),
    };
}
...
test "comint export ids are appended after terminal ABI ids" {
    ...
    try std.testing.expectEqual(@as(u32, 36), @intFromEnum(ExportId.comint_set_palette));
    try std.testing.expectEqual(@as(u32, 38), @intFromEnum(ExportId.write_pty));
}
