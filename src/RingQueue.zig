...
/// `head` in order and only reads a slot after observing it ready. `tail` is the
/// published tail used by the consumer and by producers when checking capacity.
/// It is not safe for multiple consumers or arbitrary list-style mutation.
///
...
pub fn RingQueue(comptime T: type, comptime capacity: usize) type {
    ...
    return struct {
        ...
        pub fn pushReserved(self: *Self, item: T, reserved_slots: usize) bool {
            ...
            var publish = slot_index;
            while (self.tail.load(.acquire) == publish and self.slots[publish].ready.load(.acquire)) {
                const new_tail = advance(publish);
                _ = self.tail.cmpxchgStrong(publish, new_tail, .acq_rel, .monotonic) orelse {
                    publish = new_tail;
                    continue;
                };
                break;
