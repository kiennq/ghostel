...
fn readOutput(
    ...
    while (true) {
        ...
        switch (wait_result) {
            .ready => {
                const complete_result = try completeOverlapped(self.pty_output, &overlapped, false);
                if (complete_result == .bytes) {
                    stream.nextSlice(buf[0..complete_result.bytes]);
                    return .output;
                }
                return .finished;
            },
            ...
            .command, .stopped => {
                const complete_result = try completeOverlapped(self.pty_output, &overlapped, true);
