...
pub fn init(alloc: Allocator, initial_cols: u16, initial_rows: u16, params: ProcessParams) !Self {
    ...
    if (state.output_read_event == null) {
...
pub fn drain(self: *Self, stream: anytype) !bool {
    ...
    while (self.state.running.load(.acquire)) {
        switch (try readOutput(self.state, stream, buf[0..], !drained)) {
            .output => drained = true,
...
fn readOutput(
    ...
    switch (c.GetLastError()) {
        c.ERROR_IO_PENDING => {},
        c.ERROR_OPERATION_ABORTED, c.ERROR_BROKEN_PIPE, c.ERROR_INVALID_HANDLE => return .eof,
        else => return error.ReadFailed,
    }

    if (!wait) return cancelPendingRead(state, stream, buf, &overlapped);

    var include_process = true;
    while (true) {
        switch (try waitForReadOrProcess(state, include_process)) {
...
fn cancelPendingRead(
    ...
    if (c.GetOverlappedResult(
        ...
        const err = c.GetLastError();
        switch (err) {
            c.ERROR_OPERATION_ABORTED => return .would_block,
...
fn waitForReadOrProcess(state: *State, include_process: bool) !WaitResult {
...
pub fn requestStop(self: *Self, read_thread: std.Thread) void {
    ...
    if (self.state.pty_output != c.INVALID_HANDLE_VALUE) {
        _ = c.CancelIoEx(self.state.pty_output, null);
    }
...
fn closeConPtyHandles(state: *State) void {
    ...
    if (state.output_read_event != c.INVALID_HANDLE_VALUE and state.output_read_event != null) {
...
test "output pipe supports overlapped read completion" {
    var read_handle: c.HANDLE = c.INVALID_HANDLE_VALUE;
    var write_handle: c.HANDLE = c.INVALID_HANDLE_VALUE;
    try createOverlappedPipe(&read_handle, &write_handle);
    defer _ = c.CloseHandle(read_handle);
    defer _ = c.CloseHandle(write_handle);

    const event = c.CreateEventW(null, c.TRUE, c.FALSE, null);
    ...
    var bytes_read: c.DWORD = 0;
    try std.testing.expect(c.ReadFile(
        read_handle,
        &buf,
        buf.len,
        &bytes_read,
    ...
    try std.testing.expectEqual(@as(c.DWORD, @intCast(c.ERROR_IO_PENDING)), c.GetLastError());

    var bytes_written: c.DWORD = 0;
    try std.testing.expect(c.WriteFile(
        write_handle,
        "x",
        1,
        &bytes_written,
    ...
    try std.testing.expectEqual(@as(c.DWORD, 1), bytes_written);
    try std.testing.expectEqual(c.WAIT_OBJECT_0, c.WaitForSingleObject(event, 1000));
    try std.testing.expect(c.GetOverlappedResult(
        read_handle,
        &overlapped,
        &bytes_read,
        c.FALSE,
    ) != 0);
    try std.testing.expectEqual(@as(c.DWORD, 1), bytes_read);
