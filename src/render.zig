...
pub fn redraw(env: emacs.Env, term: *Terminal, force_full_arg: bool) void {
    ...
    if (dirty != gt.DIRTY_FALSE or force_full) {
        ...
        // Remove any trailing newline so the buffer ends at the last
        // terminal row.  A stale trailing newline pushes point-max one
        // line past the viewport, and Emacs auto-scrolls to keep point
        // visible — producing visible jitter on every redraw.
        {
            const pm = env.pointMax();
            const pm_int = env.extractInteger(pm);
            if (pm_int > 1) {
                env.gotoChar(pm);
                const ch = env.extractInteger(env.call0(emacs.sym.@"char-before"));
                if (ch == '\n') {
                    env.deleteRegion(env.makeInteger(pm_int - 1), pm);
                }
            }
        }
