...
pub fn redraw(env: emacs.Env, term: *Terminal, force_full_arg: bool) void {
    ...
    if (dirty != gt.DIRTY_FALSE or force_full) {
        ...
        // Also strip any trailing newline left by the incremental path
        // (which inserts \n before appending a new row at point-max),
        // so the buffer ends exactly at the last terminal row.
