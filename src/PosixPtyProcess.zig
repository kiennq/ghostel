...
test "drain reports eof when read returns zero bytes" {
    const pipe = try posix.pipe2(.{ .CLOEXEC = true, .NONBLOCK = true });
    ...
    defer process.closeWakePipe();
    var stream = struct {
        pub fn nextSlice(_: *@This(), _: []const u8) void {
            unreachable;
        }
    }{};

    try std.testing.expect(try process.drain(&stream));
}
...
test "backend hands handles to reaper once" {
    ...
    try std.testing.expectEqual(@as(posix.fd_t, -1), process.wake_pipe[0]);
    try std.testing.expectEqual(@as(posix.fd_t, -1), process.wake_pipe[1]);
    try std.testing.expectEqual(@as(posix.fd_t, -1), backend.wake_pipe[0]);
    try std.testing.expectEqual(@as(posix.fd_t, -1), backend.wake_pipe[1]);
}
