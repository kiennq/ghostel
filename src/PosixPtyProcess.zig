...
pub const EventWriter = struct {
    ...
    pub fn blockSigpipe(self: *EventWriter) void {
...
pub fn read(self: *Self, buf: []u8) !usize {
    while (true) {
        ...
        if (pollfds[1].revents != 0) return 0;

        const len = posix.read(self.pty.primary_fd, buf) catch |err| switch (err) {
            error.WouldBlock => continue,
            error.NotOpenForReading, error.InputOutput => return 0,
            else => return err,
        };
        if (len != 0) return len;
        if (pollfds[0].revents & posix.POLL.HUP != 0) return 0;
    }
}
