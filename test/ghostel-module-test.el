...
(ert-deftest ghostel-test-extract-module-archive-uses-tar-untar-buffer ()
  "Downloaded module archives are unpacked with Emacs' `tar-mode' helper."
  (require 'tar-mode)
  (let* ((archive (expand-file-name
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'insert-file-contents)
                   (lambda (path &rest _)
                     (push (list 'insert path) events)))
                  ((symbol-function 'tar-mode)
                   (lambda ()
                     (push 'tar-mode events)))
                  ((symbol-function 'tar-untar-buffer)
                   (lambda ()
                     (push (list 'untar default-directory
                                 (not enable-multibyte-characters))
                           events))))
          (ghostel--extract-module-archive archive staging-dir)
          (should (equal `((untar ,(file-name-as-directory staging-dir) t)
                           tar-mode
                           (insert ,archive))
                         events)))
...
(ert-deftest ghostel-test-publish-downloaded-module-archive-preserves-existing-windows-backups ()
  "Downloading on Windows backs up only the loaded dyn-loader DLL."
  (let* ((archive "/ghostel/ghostel-module-x86_64-windows.tar.xz")
         ...
         (manifest-src (ghostel-test--fixture-path staging-dir "ghostel-module.json"))
    ...
    (ghostel-test--without-subr-trampolines
      (cl-letf (((symbol-function 'make-temp-file)
        ...
        (should-not (cl-find target-dest renames
                             :key #'car
                             :test #'string-equal))
        (should (equal 3 (length copies)))
        (should (equal (list staging-dir t) cleaned))))))

(ert-deftest ghostel-test-publish-built-module-artifacts-copies-windows-support ()
  "Publishing a Windows bundle preserves its side-by-side ConPTY support files."
      ...
      (let ((system-type 'windows-nt)
        ...
        (unwind-protect
            (progn
              ...
              (dolist (relative (append files '("ghostel-module.json")))
                (should (file-exists-p (expand-file-name relative module-dir)))))
...
(ert-deftest ghostel-test-publish-built-module-artifacts-rotates-existing-windows-modules ()
  "Publishing rotates only the loaded dyn-loader DLL before copying replacements."
  (let* ((source-dir (ghostel-test--fixture-dir "ghostel-build"))
         (module-dir (ghostel-test--fixture-dir "ghostel-modules"))
         ...
         (manifest-src (ghostel-test--fixture-path source-dir "ghostel-module.json"))
         (loader-dest (ghostel-test--fixture-path module-dir "dyn-loader-module.dll"))
         (module-dest (ghostel-test--fixture-path module-dir "ghostel-module.dll"))
         (manifest-dest (ghostel-test--fixture-path module-dir "ghostel-module.json"))
         (loader-backup (concat loader-dest ".bak"))
        ...
        (should (member (list (downcase loader-dest)
                        ...
                        (mapcar (lambda (entry)
                                  (list (downcase (nth 0 entry))
                                        ...
                                        (nth 2 entry)))
                                renames)))
        (should-not (cl-find module-dest renames
                             :key #'car
                             :test #'string-equal))
        (should (equal 3 (length copies)))
        (should (member (list manifest-src manifest-dest t) copies))))))
