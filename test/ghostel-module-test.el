    ...
    (let ((system-type 'windows-nt)
      ...
      (ghostel-test--without-subr-trampolines
        (cl-letf (((symbol-function 'window-body-height)
                  ...
                  ((symbol-function 'ghostel--resolve-local-executable)
                   (lambda (program)
                     (cond
                      ...
                      ((equal "C:/Program Files/Emacs/cmdproxy.exe" program)
                       program)
                      (t
                       (ert-fail
          ...
          (should (equal '("C:/Program Files/Emacs/cmdproxy.exe")
                         captured-command)))))))
  ...
  (let* ((archive "C:/ghostel/ghostel-module-x86_64-windows.tar.xz")
    ...
    (ghostel-test--without-subr-trampolines
      (cl-letf (((symbol-function 'make-temp-file)
        ...
        (should (equal '("ghostel-module.json") metadata-writes))
...
(ert-deftest ghostel-test-source-groups-transport-before-internal-variables ()
  "Transport helpers live before internal variables."
  (let* ((source (ghostel-test--ghostel-source))
    ...
    (should internal-pos)
    (should (< download-pos transport-pos internal-pos))
    (dolist (marker '("(defun ghostel--native-runtime-ready-p"
                      "(defun ghostel--process-live-p"
                      "(defun ghostel--process-send"))
      (let ((pos (ghostel-test--source-pos source marker)))
        (should pos)
        (should (< download-pos pos internal-pos))))
    (should-not (string-match-p (concat "conpty" "--") source))
    (should-not (string-match-p (concat "conpty" "-module") source))
    (should-not (string-match-p (concat "ghostel--" "conpty") source))))

(ert-deftest ghostel-test-source-start-process-uses-shared-state-helper ()
  "Process startup builds shared state before backend dispatch."
  (let ((source (ghostel-test--ghostel-source)))
    ...
    (should (ghostel-test--source-pos source "(ghostel--spawn-via-native"))
    (should-not (ghostel-test--source-pos source "(defun ghostel--start-process-windows"))
    (should-not (ghostel-test--source-pos source "(ghostel--start-process-windows"))))
  ...
  (let* ((source-dir (ghostel-test--fixture-dir "ghostel-build"))
    ...
    (ghostel-test--without-subr-trampolines
      (cl-letf (((symbol-function 'file-exists-p)
        ...
        (should (equal (list module-dest loader-dest) deletes))
        ...
        (should (member (list (downcase module-dest)
                        ...
                        (mapcar (lambda (entry)
                                ...
                                renames)))
        (should (equal 2 (length copies)))
        (should (equal (list (list "ghostel-module.json"
                                   module-dir
