...
(ert-deftest ghostel-test-module-version-newer-than-minimum ()
  ...
  (dolist (module-version '("0.3.0" "0.2.0.162.ec928f"))
    (let ((warned nil)
          (ghostel--minimum-module-version "0.2.0"))
      (cl-letf (((symbol-function 'ghostel--module-version)
                 (lambda () module-version))
                ((symbol-function 'display-warning)
                 (lambda (&rest _) (setq warned t))))
        (ghostel--check-module-version "/tmp")
        (should-not warned)))))
...
(ert-deftest ghostel-test-module-download-version-accepts-release-build-suffix ()
  "Release build suffixes are ignored only for minimum-version comparison."
  (let ((ghostel--minimum-module-version "0.56.0"))
    (cl-letf (((symbol-function 'read-string)
               (lambda (&rest _) "0.56.0.162.d652b3f")))
      (should (equal "0.56.0.162.d652b3f"
