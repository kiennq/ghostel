      ...
      (ghostel-test--without-subr-trampolines
        (cl-letf (((symbol-function 'window-body-height)
                  ...
                  ((symbol-function 'ghostel--resolve-local-executable)
                   (lambda (program)
                     (should (equal "cmdproxy.exe" program))
                     "C:/Program Files/Emacs/cmdproxy.exe"))
          ...
          (should (equal (combine-and-quote-strings
                          '("C:/Program Files/Emacs/cmdproxy.exe") " ")
                         captured-command))
                         ...
                         (and initialized
                              (memq symbol
                                    (append
                                     ...
                                     ghostel--conpty-runtime-required-functions))))))
...
(ert-deftest ghostel-test-native-runtime-ready-requires-all-conpty-exports ()
  "A Windows runtime is incomplete when any ConPTY export is missing."
  (let ((system-type 'windows-nt))
    (cl-letf (((symbol-function 'fboundp)
               (lambda (symbol)
                 (not (eq symbol 'conpty--write))))
              ((symbol-function 'ghostel--native-runtime-reloadable-p)
               (lambda () t)))
      (should-not (ghostel--native-runtime-ready-p)))))
