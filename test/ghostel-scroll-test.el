...
(ert-deftest ghostel-test-pixel-anchor-uses-live-forward-measurement ()
  "A stable live anchor uses a numeric FROM measurement and keeps its result."
  (with-temp-buffer
    (dotimes (_ 8)
      (insert "stable row\n"))
    (goto-char (point-min))
    ...
    (let ((start (point))
      ...
      (cl-letf* (((symbol-function 'window-body-height)
                  (lambda (&optional _window _pixelwise) 100))
                 ...
                 ((symbol-function 'window-vscroll)
                  (lambda (&optional _window _pixels-p) 7))
                 ((symbol-function 'ghostel--window-anchored-p)
                  (lambda (&rest _args) t))
                 ((symbol-function 'window-text-pixel-size)
                  (lambda (_window from to &rest _args)
                    (push (list from to) calls)
                    (if (consp from)
                        (list 0 120 start)
                      '(0 . 107)))))
        (should (equal (list start 7 107)
                       (ghostel--pixel-anchor (selected-window) target)))
        (should (equal (list (list start target)) (nreverse calls)))))))

(ert-deftest ghostel-test-pixel-anchor-forwards-then-falls-back ()
  "A changed live layout falls back to the exact backward measurement."
  (with-temp-buffer
    (dotimes (_ 8)
      (insert "changed row\n"))
    (goto-char (point-min))
                 ...
                 ((symbol-function 'window-text-pixel-size)
                  (lambda (_window from to &rest _args)
                    ...
                    (if (consp from)
                        (list 0 120 start)
                      '(0 . 115)))))
        (should (equal (list start 20 120)
                       (ghostel--pixel-anchor (selected-window) target)))
        (should (equal (list (list start target)
                             (list (cons target -100) target))
                       (nreverse calls)))))))

(ert-deftest ghostel-test-pixel-anchor-far-history-skips-forward-measurement ()
  "A distant history start goes directly to the exact backward measurement."
  (with-temp-buffer
    (dotimes (_ 200)
      (insert "history row\n"))
    (let ((start (point-min))
      ...
      (cl-letf* (((symbol-function 'window-body-height)
                 ...
                 ((symbol-function 'window-text-pixel-size)
                  (lambda (_window from to &rest _args)
                    ...
                    (if (consp from)
                        (list 0 120 start)
                      (ert-fail "numeric FROM must be skipped")))))
        (should (equal (list start 20 120)
                       (ghostel--pixel-anchor (selected-window) target)))
        (should (equal (list (list (cons target -100) target))
                       (nreverse calls)))))))

(ert-deftest ghostel-test-pixel-anchor-non-bol-skips-forward-measurement ()
  "A non-line-boundary start goes directly to the exact backward measurement."
  (with-temp-buffer
    (dotimes (_ 8)
      (insert "non-bol row\n"))
...
(ert-deftest ghostel-test-pixel-anchor-row-hint-is-buffer-scoped ()
  "A row-change hint for one buffer never suppresses another buffer's probe."
  (let ((hint-buffer (generate-new-buffer " *ghostel-test-row-hint*"))
        (buf (generate-new-buffer " *ghostel-test-row-hint-other*"))
        (win (selected-window))
        (orig-buf (window-buffer (selected-window))))
    (unwind-protect
        (progn
          (set-window-buffer win buf)
          (with-current-buffer hint-buffer
            (ghostel-mode))
          (with-current-buffer buf
            (ghostel-mode)
            (let ((inhibit-read-only t))
              (dotimes (_ 8)
                (insert "other buffer row\n")))
            (goto-char (point-min))
            (set-window-start win (point-min) t)
            (let ((numeric-calls 0)
                  (backward-calls 0)
                  (ghostel--pixel-anchor-supported-p t)
                  (ghostel--redraw-row-geometry-changed-buffer
                   hint-buffer))
              (cl-letf* (((symbol-function 'display-graphic-p)
                         ...
                         ((symbol-function 'window-vscroll)
                          (lambda (&optional _window _pixels-p) 0))
                         ((symbol-function 'window-text-pixel-size)
                          (lambda (_window from _to &rest _args)
                            (if (consp from)
                                (progn
                                  (cl-incf backward-calls)
                                  (list 0 80 (point-min)))
                              (cl-incf numeric-calls)
                              '(0 . 80)))))
                (should (equal (list (point-min) 0 80)
                               (ghostel--pixel-anchor win (point-max))))
                (should (= 1 numeric-calls))
                (should (= 0 backward-calls))))))
      (set-window-buffer win orig-buf)
      (kill-buffer hint-buffer)
      (kill-buffer buf))))

(defun ghostel-test-scroll--pixel-anchor-reference (window target)
  "Return the original exact pixel anchor for WINDOW and TARGET."
  (let* ((body-height (window-body-height window t))
         (size (window-text-pixel-size
                window (cons target (- body-height)) target nil nil)))
    (list (nth 2 size)
          (max 0 (- (nth 1 size) body-height))
          (nth 1 size))))

(ert-deftest ghostel-test-pixel-anchor-forced-long-history-matches-reference ()
  "A graphical forced bottom anchor matches the original from deep history."
  (skip-unless (display-graphic-p))
  (let ((buf (generate-new-buffer " *ghostel-test-pixel-anchor-gui*"))
        (win (selected-window))
        (orig-buf (window-buffer (selected-window))))
    (unwind-protect
        (progn
          ...
          (with-current-buffer buf
            ...
            (let* ((target (point-max))
                   (want (ghostel-test-scroll--pixel-anchor-reference
                          win target))
                   (got (ghostel--pixel-anchor win target)))
              (should (equal want got)))))
      (set-window-buffer win orig-buf)
      (kill-buffer buf))))

(defmacro ghostel-test-scroll--with-redraw-stub (spec &rest body)
  "Run BODY in a real Ghostel buffer with the native redraw stubbed.
SPEC is (BUFFER ACTION); ACTION is called by the renderer stub."
  (declare (indent 1))
  (pcase-let ((`(,buffer ,action) spec))
    `(let ((,buffer (generate-new-buffer " *ghostel-test-redraw-anchor*"))
           (orig-buf (window-buffer (selected-window)))
             ...
             (with-current-buffer ,buffer
               ...
               (cl-letf* (((symbol-function 'ghostel--terminal-live-p)
                          ...
                          ((symbol-function 'ghostel--line-mode-post-redraw)
                           #'ignore)
                          ((symbol-function 'ghostel--line-mode-snapshot)
                           (lambda () nil))
                          ((symbol-function 'ghostel--line-mode-restore)
                           (lambda (_snapshot) nil))
...
(ert-deftest ghostel-test-redraw-row-append-skips-forward-pixel-probe ()
  "Appending a row bypasses the speculative forward pixel measurement."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window))
          (numeric-calls 0)
          (backward-calls 0)
          (row-appended nil)
      ...
      (setq start-before (window-start win)
            render-action
            (lambda ()
              (setq row-appended t)
              (goto-char (point-max))
      ...
      (let ((ghostel--pixel-anchor-supported-p t))
        (cl-letf* (((symbol-function 'display-graphic-p)
                    (lambda (&rest _args) t))
                   ((symbol-function 'window-body-height)
                    (lambda (&optional _window _pixelwise) 230))
                   ((symbol-function 'default-line-height)
                    (lambda () 10))
                   ((symbol-function 'window-vscroll)
                    (lambda (&optional _window _pixels-p) 7))
                   ((symbol-function 'window-text-pixel-size)
                    (lambda (_window from _to &rest _args)
                      (if (consp from)
                          (progn
                            (cl-incf backward-calls)
                            (list 0 237 start-before))
                        (cl-incf numeric-calls)
                        (if row-appended
                            '(0 . 230)
                          '(0 . 237))))))
          (ghostel--redraw-now buf)))
      (should (= 0 numeric-calls))
      (should (= 1 backward-calls))
      (should (= start-before (window-start win))))))

(ert-deftest ghostel-test-redraw-inline-typing-keeps-forward-pixel-probe ()
  "Inline typing without a row change keeps the numeric pixel measurement."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window))
          (numeric-calls 0)
          (backward-calls 0)
...
(ert-deftest ghostel-test-redraw-prefix-eviction-skips-forward-pixel-probe ()
  "Prefix eviction plus a replacement row bypasses forward measurement."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window))
          (numeric-calls 0)
          (backward-calls 0)
...
(defmacro ghostel-test-scroll--with-start-spy (calls &rest body)
  "Run BODY while recording every `set-window-start' call in CALLS."
  (declare (indent 1))
  `(let ((orig-set-window-start (symbol-function 'set-window-start)))
     (cl-letf (((symbol-function 'set-window-start)
                (lambda (window start &rest args)
                  (push (list window start) ,calls)
                  (apply orig-set-window-start window start args))))
       ,@body)))

(ert-deftest ghostel-test-redraw-same-start-skips-setter-and-updates-point ()
  "A zero-mutation redraw skips the same start but still updates point."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window)))
      (ghostel--anchor-window win t)
      (goto-char (point-min))
      ...
      (let ((start (window-start win))
            ...
            (starts nil))
        (setq render-action (lambda () nil))
        (ghostel-test-scroll--with-start-spy starts
          (ghostel--redraw-now buf))
        (should-not starts)
        (should (= start (window-start win)))
        (should (= cursor (window-point win)))))))

(ert-deftest ghostel-test-redraw-text-and-property-mutations-retain-setter ()
  "Text and property-only renderer mutations retain start positioning."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window)))
      (dolist (mutation
               (list (lambda ()
                     ...
                     (lambda ()
                       (put-text-property
                        (point-min) (1+ (point-min)) 'face 'bold))))
        (ghostel--anchor-window win t)
        (setq render-action mutation)
        (let ((starts nil))
          (ghostel-test-scroll--with-start-spy starts
            (ghostel--redraw-now buf))
          (should starts))))))

(ert-deftest ghostel-test-redraw-force-policies-retain-setter ()
  "Force, full, recovery, and line-mode redraws retain start positioning."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window)))
      (setq render-action (lambda () nil))
      (dolist (policy '(force full recovery line))
        (ghostel--anchor-window win t)
        (let ((starts nil))
          (ghostel-test-scroll--with-start-spy starts
            (pcase policy
              ...
              ('full
               (let ((ghostel-full-redraw t))
                 (ghostel--redraw-now buf)))
              ('recovery
               (setq-local ghostel--force-next-redraw t)
               (ghostel--redraw-now buf))
              ('line
...
(ert-deftest ghostel-test-anchor-start-guard-requires-same-scope-and-position ()
  "The dynamic same-start guard does not leak or hide changed positions."
  (ghostel-test-scroll--with-redraw-stub (buf render-action)
    (let ((win (selected-window))
          (other (generate-new-buffer " *ghostel-test-other-buffer*")))
      (unwind-protect
          (progn
            (setq render-action (lambda () nil))
            (ghostel--anchor-window win t)
            (let ((starts nil))
              (let ((ghostel--redraw-unchanged-buffer buf))
                (ghostel-test-scroll--with-start-spy starts
                  (ghostel--anchor-window win)))
              (should-not starts))
            (let ((starts nil))
              (let ((ghostel--redraw-unchanged-buffer other))
                (ghostel-test-scroll--with-start-spy starts
                  (ghostel--anchor-window win)))
              (should starts))
            (let ((starts nil))
