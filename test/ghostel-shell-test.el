...
(defun ghostel-test--bash-source-path (path)
  ...
  (let* ((quoted (ghostel--posix-shell-quote-argument path))
    ...
    (with-temp-buffer
      (when (and (zerop (call-process "bash" nil t nil
    ...
    (unwind-protect
        (with-current-buffer buf
          ...
          (let* ((process-environment
            ...
            ;; Use letters outside [0-9a-f]: fish's default prompt shows the
            ;; checkout's VCS state, and on a detached HEAD that may be a short
            ;; hexadecimal commit id that never leaves the screen.
            (process-send-string proc "wxyz")
            (ghostel-test--wait-for
             proc
             (lambda ()
               (string-match-p "wxyz"
                               (or (ghostel--copy-all-text ghostel--term)
                                   ""))))
            (let ((state (ghostel--copy-all-text ghostel--term)))
              (should (string-match-p "wxyz" state)))
...
(ert-deftest ghostel-test-password-detect-regex-fallback ()
  "Regex fallback fires when heuristic returns nil and fallback is allowed.
...
stubbed nil and `ghostel--password-regex-fallback-p' stubbed t."
  :tags '(native)
    ...
    (unwind-protect
        (with-current-buffer buf
          ...
          (cl-letf (((symbol-function 'ghostel--pty-password-input-p)
                     (lambda (_term) nil))
                    ((symbol-function 'ghostel--password-regex-fallback-p)
                     (lambda () t)))
            (should (eq (ghostel--password-prompt-detected-p) 'regex))))
      (when (buffer-live-p buf) (kill-buffer buf)))))

(ert-deftest ghostel-test-password-detect-regex-fallback-on-windows ()
  "Windows uses the regex fallback because ConPTY has no termios state."
  :tags '(native)
  (let ((buf (generate-new-buffer " *ghostel-test-pwd-regex-windows*")))
    (unwind-protect
        (with-current-buffer buf
          (ghostel-mode)
          (setq ghostel--term (ghostel--new 5 80 1000))
...
(ert-deftest ghostel-test-password-detect-fallback-without-pty-probe ()
  "Password fallback works when the local pty probe is unavailable."
  :tags '(native)
  (let ((buf (generate-new-buffer " *ghostel-test-pwd-remote-skip-pty*")))
    (unwind-protect
        (with-current-buffer buf
          ...
          (let ((inhibit-read-only t))
            (ghostel--redraw ghostel--term t))
          (cl-letf (((symbol-function 'ghostel--pty-password-input-p) nil)
                    ((symbol-function 'ghostel--password-regex-fallback-p)
                     (lambda () t)))
            (should (eq (ghostel--password-prompt-detected-p) 'regex))))
      (when (buffer-live-p buf) (kill-buffer buf)))))
...
`read-passwd' prompt is avoided.  Stubs
`ghostel--password-regex-fallback-p' to t so the regex arm is even
reachable."
  :tags '(native)
