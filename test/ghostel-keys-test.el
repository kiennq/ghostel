...
(ert-deftest ghostel-test-send-event-encoded-specials-use-process-transport ()
  "`ghostel--send-event' special keys must use the process transport boundary."
  :tags '(native)
  (with-temp-buffer
    (let* ((term (ghostel--new 25 80 1000))
      ...
      (cl-letf (((symbol-function 'process-live-p) (lambda (_process) t))
                ...
                ((symbol-function 'ghostel--process-send)
                 (lambda (_process data) (push data sent)))
...
(ert-deftest ghostel-test-encode-key-kitty-backspace ()
  ...
  (let* ((term (ghostel--new 25 80 1000))
    ...
    ;; Capture what the native encoder writes through the process transport.
    (cl-letf (((symbol-function 'process-live-p) (lambda (_process) t))
              ...
              ((symbol-function 'process-send-string)
               (lambda (&rest args)
                 (ert-fail
                  (format "unexpected direct process-send-string: %S" args)))))
...
(ert-deftest ghostel-test-flush-output-uses-process-transport ()
  "`ghostel--flush-output' writes through the process transport boundary.
The native module calls this helper for encoded keys and other direct PTY
writes, so Windows ConPTY must not fall back to direct `process-send-string'."
  :tags '(native)
  (with-temp-buffer
  ...
  (with-temp-buffer
    (let ((ghostel--term 'fake)
      ...
      (cl-letf (((symbol-function 'ghostel--process-live-p) (lambda (&optional _p) t))
                ((symbol-function 'ghostel--process-send)
                 (lambda (_process str) (push str sent))))
  ...
  (with-temp-buffer
    (let ((ghostel--term 'fake)
      ...
      (cl-letf (((symbol-function 'ghostel--encode-key)
                 (lambda (_term _key _mods &optional _utf8) nil))
                ((symbol-function 'ghostel--process-live-p) (lambda (&optional _p) t))
                ((symbol-function 'ghostel--process-send) #'ignore))
        (ghostel--send-encoded "backspace" "")
        (should ghostel--last-send-time)))))
