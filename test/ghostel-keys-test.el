...
(ert-deftest ghostel-test-invalidate-visible-background-buffer-schedules-redraw ()
  "Visible output in an unselected window is scheduled for redraw."
  (with-temp-buffer
...
(ert-deftest ghostel-test-invalidate-invisible-buffer-remains-pending ()
  "A window on an invisible frame does not schedule a redraw."
  (with-temp-buffer
    (let ((ghostel--redraw-timer 'existing-timer)
      ...
      (cl-letf (((symbol-function 'ghostel--windows)
                 (lambda (&rest _) (list hidden-window)))
                ((symbol-function 'window-frame)
                 (lambda (_window) 'hidden-frame))
                ((symbol-function 'frame-visible-p)
                 (lambda (_frame) nil))
                ((symbol-function 'display-graphic-p)
                 (lambda (&rest _) nil))
                ((symbol-function 'selected-window)
                 (lambda () hidden-window))
...
(ert-deftest ghostel-test-invalidate-visible-different-buffer-does-not-qualify-this-buffer ()
  "A visible window for another Ghostel buffer does not schedule this buffer."
  (with-temp-buffer
    (let ((other (generate-new-buffer " *ghostel-test-other-visible*"))
      ...
      (unwind-protect
          (cl-letf (((symbol-function 'ghostel--windows)
                     (lambda (buffer &rest _)
                       (when (eq buffer other) '(other-window))))
                    ((symbol-function 'window-frame)
                     (lambda (&rest _) 'visible-frame))
                    ((symbol-function 'frame-visible-p)
                     (lambda (&rest _) t))
                    ((symbol-function 'display-graphic-p)
                     (lambda (&rest _) nil))
...
(ert-deftest ghostel-test-invalidate-copy-mode-stays-pending-without-timer ()
  "Visible copy-mode output is deferred like output for a hidden buffer."
  (with-temp-buffer
