...
(defun ghostel-test--insert-rendered (&rest args)
  "Insert rendered fixture ARGS into a read-only Ghostel buffer."
  (let ((inhibit-read-only t))
    (apply #'insert args)))
...
Signal an ERT failure if TIMEOUT is reached or PRED remains nil after
PROC exits."
  (let* ((timeout (or timeout 5))
    ...
    (unless result
      ;; A fast process can exit between the predicate check and the next
      ;; poll; give Emacs one final chance to run filters/sentinels.
      (accept-process-output proc 0.05)
      (setq result (funcall pred)))
    (unless result
      (ert-fail
  ...
  (let* ((root (or (ghostel--resource-root)
    ...
    (skip-unless
     (zerop (call-process "bash" nil nil nil
                          "--noprofile" "--norc" "-c"
                          (format "test -r %s"
                                  (shell-quote-argument shell-bash)))))
    (let* ((probe
            (concat
             "PROMPT_COMMAND='printf \"\\e]7;file://competing-host/path\\a\"';"
             (format " source %s;" (shell-quote-argument shell-bash))
...
(ert-deftest ghostel-test-url-detection ()
  ...
  ;; Keep point on a trailing empty line so the cursor-row guard does not
  ;; mask historical output while these tests inspect linkification.
  (with-temp-buffer
    (insert "Visit https://example.com and http://test.org now\n")
    ...
    (search-forward "http://test.org")
    (should (equal "http://test.org"
                   (get-text-property (match-beginning 0) 'help-echo))))

  (with-temp-buffer
    (insert "Visit https://example.com\n")
  ...
  (with-temp-buffer
    (insert "Visit https://example.com\n")
    (add-text-properties 7 26 '(help-echo "existing" mouse-face highlight))
    (let ((ghostel-enable-url-detection t))
      (ghostel--detect-plain-links))
    (should (equal "existing" (get-text-property 10 'help-echo))))

  (with-temp-buffer
    (insert "URL: https://example.com/path?x=1, and https://test.org.\n")
    (let ((ghostel-enable-url-detection t))
      (ghostel--detect-plain-links))
    (goto-char (point-min))
    (search-forward "https://example.com/path?x=1")
    (should (equal "https://example.com/path?x=1"
                   (get-text-property (match-beginning 0) 'help-echo)))
    (search-forward "https://test.org")
    (should (equal "https://test.org"
                   (get-text-property (match-beginning 0) 'help-echo))))
  ...
  (let* ((test-file (locate-library "ghostel"))
    ...
    (cl-labels ((find-fileref
                 (predicate)
                 (let ((pos (point-min)) found)
                   (while (and (not found) (< pos (point-max)))
                     (let ((help (get-text-property pos 'help-echo)))
                       (when (and (stringp help)
                                  (string-prefix-p "fileref:" help)
                                  (funcall predicate help pos))
                         (setq found help)))
                     (setq pos (1+ pos)))
                   found)))
      (with-temp-buffer
        (insert (format "File: %s:42\n" test-file))
        (let ((ghostel-enable-url-detection nil)
              (ghostel-enable-file-detection t))
          (ghostel--detect-plain-links))
        (let ((help (find-fileref (lambda (_help pos)
                                    (get-text-property pos 'keymap)))))
          (should help)
          (should (string-match-p (regexp-quote test-file) help))
          (should (string-match-p ":42" help))))

      (with-temp-buffer
        (let ((default-directory test-dir))
          (insert (format "Relative file: ./%s:10\n" test-name))
          (let ((ghostel-enable-url-detection nil)
                (ghostel-enable-file-detection t))
            (ghostel--detect-plain-links))
          (let ((help (find-fileref (lambda (_help pos)
                                      (get-text-property pos 'keymap)))))
            (should help)
            (should (string-match-p (regexp-quote test-name) help))
            (should (string-match-p ":10" help)))))

      (with-temp-buffer
        (let ((home-dir (file-name-as-directory (expand-file-name "~"))))
          (insert "Home file: ~/test-file.txt:5\n")
          ...
          (let ((help (find-fileref (lambda (_help pos)
                                      (get-text-property pos 'keymap)))))
            (should help)
            (should (string-match-p (regexp-quote (expand-file-name "~/test-file.txt")) help))
            (should (string-match-p ":5" help)))))

      (with-temp-buffer
        (insert "Missing file still gets a pattern link: /definitely/missing/file.txt:99\n")
        (let ((ghostel-enable-url-detection nil)
              (ghostel-enable-file-detection t))
          (ghostel--detect-plain-links))
        (let ((help (find-fileref (lambda (_help pos)
                                    (get-text-property pos 'keymap)))))
          (should help)
          (should (string-match-p "/definitely/missing/file.txt" help))
          (should (string-match-p ":99" help))))
      ...
      (with-temp-buffer
        ...
        (let ((ghostel-enable-url-detection nil)
              (ghostel-enable-file-detection nil))
          (ghostel--detect-plain-links))
        (should-not (find-fileref (lambda (_help pos)
                                    (get-text-property pos 'keymap))))))))
          ...
          (let ((ghostel-password-prompt-functions
            ...
            (cl-letf (((symbol-function 'ghostel--password-prompt-detected-p)
              ...
              (let ((deadline (+ (float-time) 1.0)))
                (while (and (= 0 calls) (< (float-time) deadline))
...
flag-flip tests don't leak processes between runs."
  ...
  `(let* ((buf (generate-new-buffer " *ghostel-test-query-cat*"))
          (,var (make-pipe-process :name "ghostel-test-cat"
                                   :buffer buf
                                   :noquery nil)))
...
Run a long-lived shell loop interactively, verify both conditions,
then send bytes through the process to confirm they land in the buffer."
  (skip-unless (file-executable-p "/bin/sh"))
    ...
    (unwind-protect
        (let ((buf (ghostel-compile--start command buf-name
          ...
          (with-current-buffer buf
            ...
            ;; Round-trip: send a line, expect the loop to print it back.
            ...
            (ghostel-test--wait-for
             ...
             (lambda ()
               (cl-some (lambda (s) (string-match-p "ghosttel-ping" s))
                        ghostel--pending-output)))
            ;; Shut the loop down so the test doesn't leak a process.
            (process-send-string ghostel--process "ghostel-stop\n")
            (ghostel-test--wait-for
             ghostel--process
  ...
  (let* ((buf-name "*ghostel-test-kill-compilation*")
    ...
    (when (get-buffer buf-name)
      (let ((kill-buffer-query-functions nil))
        (kill-buffer buf-name)))
    (unwind-protect
        (let ((buf (ghostel-compile--start "/bin/sleep 600" buf-name
                                           default-directory)))
          (with-current-buffer buf
            (ghostel-test--wait-for
             ghostel--process
            ...
            ;; and wait for the process to die via SIGINT.  `/bin/sleep' exits
            ;; on SIGINT, the sentinel finalizes, and `--last-exit' reflects
...
(ert-deftest ghostel-test-module-directory-honours-custom-value ()
  ...
  (let ((ghostel-module-dir nil)
        (ghostel-module-directory "~/custom/ghostel/"))
    (cl-letf (((symbol-function 'ghostel--resource-root)
                (lambda () "/pkg/ghostel/")))
      (should (equal (downcase (file-name-as-directory
...
(ert-deftest ghostel-test-load-module-looks-in-custom-directory ()
  ...
  (let* ((ghostel-module-dir nil)
         ...
         (loader-path (expand-file-name "dyn-loader-module.so" "/custom/dir/"))
         (manifest-path (expand-file-name "ghostel-module.json" "/custom/dir/"))
         (target-path (expand-file-name "ghostel-module.so" "/custom/dir/"))
         (loaded-path nil)
         (manifests nil)
         (checked nil)
        ...
        (progn
          ...
          (cl-letf (((symbol-function 'ghostel--resource-root)
                     ...
                     ((symbol-function 'file-exists-p)
                      (lambda (path)
                        (member (downcase path)
                                (mapcar #'downcase
                                        (list loader-path manifest-path target-path)))))
                    ((symbol-function 'featurep)
                     (lambda (feature)
                       (and (not (memq feature '(ghostel-module dyn-loader-module)))
                    ...
                    ((symbol-function 'ghostel--loader-load-manifest)
                     (lambda (path) (push path manifests)))
                     ((symbol-function 'ghostel--check-module-version)
    ...
    (should (equal (downcase loader-path)
                   (downcase loaded-path)))
    (should (equal (list (downcase manifest-path))
  ...
  (let ((warnings nil))
    (let ((native-comp-enable-subr-trampolines nil))
      (cl-letf (((symbol-function 'ghostel--resource-root)
        ...
        (should-not (ghostel--compile-module "/src/ghostel/"))
        (should warnings)))))
...
already loaded, so this test temporarily hides `ghostel--new' and
the native module feature flags to force the missing-file code path,
then restores them."
  ...
  (let* ((tmp (make-temp-file "ghostel-test-load-stale" t))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'module-load)
                     (lambda (path) (push path load-calls)))
                    ((symbol-function 'ghostel--loader-load-manifest)
                     (lambda (path) (push path manifest-calls)))
                    ((symbol-function 'ghostel--ensure-module)
  ...
  (let ((buf (generate-new-buffer " *ghostel-test-line-send-dedup*"))
    ...
    (unwind-protect
        (with-current-buffer buf
          ...
          (ghostel-test--insert-rendered
           (propertize "ls -la" 'ghostel-input t))
          (let ((ghostel--term 'fake)
                (ghostel--process 'fake-proc))
            (cl-letf (((symbol-function 'ghostel--mode-enabled)
                       (lambda (&rest _) nil))
  ...
  (let ((buf (generate-new-buffer " *ghostel-test-line-adopt*")))
    (unwind-protect
        (with-current-buffer buf
          ...
          (ghostel-test--insert-rendered
           (propertize "cd src" 'ghostel-input t))
          (let ((ghostel--term 'fake)
                (ghostel--process 'fake-proc))
            (cl-letf (((symbol-function 'ghostel--mode-enabled)
                       (lambda (&rest _) nil))
...
and absent (fall back to `xterm-256color' so echo works)."
  ...
  (let* ((ghostel-term "xterm-256color")
    ...
    (should (string-match-p
             (regexp-quote (concat "TERM=" (shell-quote-argument "xterm-256color")))
             preamble))
    (should (string-match-p "COLORTERM=truecolor" preamble)))
  (let* ((ghostel-term "screen-256color")
    ...
    (should-not (string-match-p "infocmp" preamble))
    (should (string-match-p
             (regexp-quote (concat "TERM=" (shell-quote-argument "screen-256color")))
             preamble))))
  ...
  (let ((captured-env nil)
    ...
    (cl-letf (((symbol-function #'make-process)
              ...
              ((symbol-function #'make-pipe-process)
               (lambda (&rest plist)
                 (ignore plist)
                 (funcall orig-make-pipe-process
                          :name "ghostel-test-fake"
                          :buffer (current-buffer)
                          :noquery t
                          :filter #'ignore
                          :sentinel #'ignore)))
              ((symbol-function 'conpty--init)
               (lambda (_term _proc _shell _height _width _cwd env)
...
so no actual shell is spawned."
  (let ((native-comp-enable-subr-trampolines nil)
        ...
        (inhibit-message t)
        (orig-process-command (symbol-function #'process-command)))
    (cl-letf (((symbol-function #'make-process)
                (lambda (&rest plist)
                  (let ((proc (make-pipe-process
                               :name "ghostel-test-debug-spawn"
                               :buffer (plist-get plist :buffer)
                               :noquery t
                               :filter #'ignore
                               :sentinel #'ignore)))
                    (process-put proc 'ghostel-test-command
                                 '("true"))
                    proc)))
               ((symbol-function #'process-command)
                (lambda (proc)
                  (or (process-get proc 'ghostel-test-command)
                      (funcall orig-process-command proc)))))
...
(defconst ghostel-test--elisp-tests
  '(ghostel-test-focus-window-selection
    ...
    ghostel-test-download-module-reloads-loaded-runtime
    ghostel-test-download-module-rejects-non-loader-runtime
    ghostel-test-download-module-rejects-unregistered-loader-runtime
    ghostel-test-download-module-closes-live-buffers-before-reload
    ghostel-test-reload-module-closes-live-buffers-before-reload
    ...
    ghostel-test-readonly-recenter
