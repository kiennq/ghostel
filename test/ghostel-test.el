...
Run a long-lived shell loop, verify both conditions, then send
bytes through the process to confirm they land in the buffer."
  ...
  (let* ((buf-name "*ghostel-test-interactive-compile*")
    ...
    (unwind-protect
        (let ((buf (ghostel-compile--start command buf-name default-directory)))
          (with-current-buffer buf
  ...
  (let* ((buf-name "*ghostel-test-kill-compilation*")
    ...
    (when (get-buffer buf-name)
      (let ((kill-buffer-query-functions nil))
        (kill-buffer buf-name)))
    (unwind-protect
        (let ((buf (ghostel-compile--start "sleep 600" buf-name
                                           default-directory)))
          (with-current-buffer buf
            (ghostel-test--wait-for
             ghostel--process
