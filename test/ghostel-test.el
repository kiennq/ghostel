    ...
    (unwind-protect
        (with-current-buffer buf
          (let* ((term (ghostel--new 5 40 100))
            ...
            ;; count-lines counts newline separators; the trailing
            ;; newline is stripped by the renderer so 3 content lines
            ;; plus one blank row yield 3 newlines.
            (should (= 3 (count-lines (point-min) (point-max)))))) ; line count
      (kill-buffer buf))))
...
(ert-deftest ghostel-test-source-groups-conpty-backend-before-internal-variables ()
  ...
  (let* ((source (ghostel-test--ghostel-source))
    ...
    (dolist (marker '("(declare-function conpty--init \"conpty-module\")"
                      ...
                      "(defun ghostel--ensure-conpty-loaded"
