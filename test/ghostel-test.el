...
(ert-deftest ghostel-test-windows-ghostel-starts ()
  "Windows `ghostel' starts a live terminal buffer."
  (skip-unless (eq system-type 'windows-nt))
  (unless (ghostel--native-runtime-ready-p)
    (should (ghostel--load-module-if-available (ghostel--effective-module-dir))))
  (skip-unless (ghostel--native-runtime-ready-p))
  (let ((ghostel-shell (or (getenv "COMSPEC") "cmd.exe"))
        (ghostel-shell-integration nil)
        (ghostel-buffer-name (generate-new-buffer-name " *ghostel-startup-test*"))
        (buf nil))
    (unwind-protect
        (progn
          (setq buf (ghostel '(4)))
      ...
      (when (buffer-live-p buf)
        (with-current-buffer buf
          (ignore-errors
            (when (and ghostel--term (fboundp 'conpty--kill))
