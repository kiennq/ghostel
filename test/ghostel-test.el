...
(ert-deftest ghostel-test-ghostel-loads-windows-runtime-before-starting ()
  "Invoking `ghostel' on Windows loads missing ConPTY runtime before start."
  (let* ((ghostel-buffer-name "*ghostel-windows-runtime*")
         (module-dir (ghostel-test--fixture-dir "ghostel-runtime"))
        ...
        (ghostel-test--without-subr-trampolines
         (let ((old-fboundp (symbol-function 'fboundp)))
           (cl-letf (((symbol-function 'fboundp)
                      (lambda (sym)
                        (if (eq sym 'conpty--init)
                            runtime-ready
                          (funcall old-fboundp sym))))
                     ((symbol-function 'ghostel--initialize-native-modules)
                      (lambda () nil))
                     ...
                     ((symbol-function 'pop-to-buffer)
                      (lambda (buffer-or-name &rest _)
                        ...
                        buf)))
             (ghostel)
             (should (equal (list (downcase module-dir))
...
(ert-deftest ghostel-test-initialize-native-modules-reloads-already-loaded-module-when-safe ()
  ...
  (let* ((module-dir (ghostel-test--fixture-dir "ghostel-init-modules"))
         (loader-path (ghostel-test--fixture-path module-dir "dyn-loader-module.dll"))
         (manifest-path (ghostel-test--fixture-path module-dir "ghostel-module.json"))
         (ghostel-module-dir module-dir)
         (module-file-suffix ".dll")
...
(ert-deftest ghostel-test-initialize-native-modules-warns-when-live-buffers-block-refresh ()
  ...
  (let* ((module-dir (ghostel-test--fixture-dir "ghostel-init-live"))
         (loader-path (ghostel-test--fixture-path module-dir "dyn-loader-module.dll"))
         (manifest-path (ghostel-test--fixture-path module-dir "ghostel-module.json"))
         (ghostel-module-dir module-dir)
         (module-file-suffix ".dll")
...
(defconst ghostel-test--elisp-tests
  '(ghostel-test-source-omits-removed-native-hooks
    ...
    ghostel-test-ghostel-clamps-initial-terminal-size-to-window-max-chars-minus-one
    ghostel-test-ghostel-loads-windows-runtime-before-starting
