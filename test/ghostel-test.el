    ...
    (unwind-protect
        (with-current-buffer buf
          (let* ((term (ghostel--new 5 80 1000))
            ...
            ;; Sanity: deferred post-processing applied a help-echo while the row
            ;; is visible.
            (goto-char (point-min))
    ...
    (unwind-protect
        (with-current-buffer buf
          (let* ((term (ghostel--new 5 40 100))
            ...
            ;; 5-row terminal: 3 content + 2 blank rows = 4 newlines.
            ;; Keep the final newline when it represents the legitimate
            ;; blank last terminal row; only truly excess trailing lines
            ;; should be trimmed.
...
(ert-deftest ghostel-test-url-detection ()
  ...
  (unless (eq system-type 'windows-nt)
    (let ((test-file (locate-library "ghostel")))
      ...
      ;; File:line detection is pattern-based and must not stat candidate paths.
      (with-temp-buffer
