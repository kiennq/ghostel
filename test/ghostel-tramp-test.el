...
(defun ghostel-tramp-test--fake-live-process (name buf)
  ...
  (let ((buf (generate-new-buffer " *ghostel-test-width-change*")))
    (unwind-protect
        (with-current-buffer buf
          ...
          (let* ((proc (ghostel-tramp-test--fake-live-process
                        "ghostel-test-w" buf))
        ...
        (with-current-buffer buf
          ...
          (let* ((process-environment
                  (append (list "TERM=xterm-256color" "COLUMNS=40" "LINES=10")
                          process-environment))
                  (proc (ghostel-tramp-test--fake-live-process
                         "ghostel-test-pipe" buf)))
