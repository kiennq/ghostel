...
(ert-deftest ghostel-test-adjust-size-passes-all-relevant-windows ()
  "`ghostel--adjust-size' passes all Ghostel windows to the adjuster."
  (with-temp-buffer
    (let ((selected-window 'selected-window)
          (other-window 'other-window)
          (cur-buf (current-buffer)))
      (dolist (case '((:window other :visible (other))
                      (:window selected :visible (selected other))
                      (:window other :visible (other))))
        (let ((ghostel--term 'fake)
          ...
          (cl-letf (((symbol-function 'ghostel--set-size)
            ...
            (should (equal visible adjust-windows))
