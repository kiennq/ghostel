  ...
  (let ((system-type 'gnu/linux)
    ...
    (cl-letf (((symbol-function 'make-process)
               (lambda (&rest args)
                 ...
                 'fake-proc))
              ((symbol-function 'ghostel--resolve-local-executable) #'identity)
      ...
      (let ((process-environment '("PATH=/usr/bin:/bin" "HOME=/tmp")))
        (ghostel--spawn-pty
         "/bin/sh" nil 25 80 ghostel--default-stty nil t)
      ...
      (let ((process-environment '("PATH=/usr/bin:/bin" "HOME=/tmp")))
        (pcase-let ((`(,program . ,args) (ghostel-test--shell-command "")))
          (ghostel--spawn-pty
           program args 25 80 ghostel--default-stty nil nil))
        (let ((script (nth 2 captured-cmd)))
...
(ert-deftest ghostel-test-start-process-remote-uses-remote-terminfo ()
    ...
    (let ((default-directory "/ssh:test@host:/tmp/")
      ...
      (cl-letf (((symbol-function #'window-body-height)
                ...
                ((symbol-function #'ghostel--spawn-process)
                 (lambda (_program _args _height _width _stty _remote-p)
                   (setq capture (copy-sequence process-environment))
                   nil)))
        (ghostel--start-process)
        (should (member "TERMINFO=/remote/tinfo" capture))
        (should-not (member "TERMINFO=/local/tinfo" capture))
        (should-not (member "TERM_PROGRAM=ghostty" capture))
        (should-not (member "GHOSTEL_SSH_INSTALL_TERMINFO=1" capture))))))

(ert-deftest ghostel-test-start-process-remote-without-terminfo-falls-back-to-xterm-256color ()
      ...
      (cl-letf (((symbol-function #'window-body-height)
                ...
                ((symbol-function #'ghostel--spawn-process)
                 (lambda (_program _args _height _width _stty _remote-p)
                   ...
                   nil)))
        (ghostel--start-process)
        (should-not (member "TERM=xterm-ghostty" capture))
        (should-not (member "TERM_PROGRAM=ghostty" capture))
  ...
  (with-temp-buffer
    (let ((selected-window 'selected-window)
      ...
      (dolist (case '((:selected-only nil :window other
                      ...
                      (:selected-only nil :window selected
                       :visible (selected other) :expect-size (40 120)
                       :expect-windows all)
                      ...
                      (:selected-only t :window other
                       ...
                       :expect-windows nil)
                      (:selected-only t :window other :force t
                       :visible (other) :expect-size nil
                       :expect-windows nil :expect-deferred t)
                      (:selected-only t :window selected
                       :visible (selected other) :expect-size (24 80)
                       :expect-windows selected :deferred t
                       :rows 24 :cols 80 :size (80 . 24)
                       :expect-deferred nil)
                      (:selected-only t :window selected
                       :visible (selected) :expect-size nil
                       :expect-windows selected :deferred t
                       :rows 40 :cols 120 :size (120 . 30)
                       :minibuffer 1 :expect-deferred t)))
        (let ((ghostel--term 'fake)
              ...
              (ghostel--deferred-force-resize
               (plist-get case :deferred))
              (adjust-windows nil)
              (set-size-args nil)
              (redraw-called nil)
          ...
          (cl-letf (((symbol-function 'ghostel--set-size)
                    ...
                    ((symbol-function 'minibuffer-depth)
                     (lambda () (or (plist-get case :minibuffer) 0)))
                    ...
                    ((default-value 'window-adjust-process-window-size-function)
                     (lambda (_proc wins)
                       ...
                       (or (plist-get case :size)
                            (if (equal wins (list selected-window))
                                '(80 . 24)
                              '(120 . 40))))))
            (ghostel--adjust-size window (plist-get case :force))
            ...
            (should (eq (plist-get case :expect-deferred)
                        ghostel--deferred-force-resize))
            (should (eq (not (null set-size-args)) redraw-called))))))))

(ert-deftest ghostel-test-selected-window-resize-catches-up-on-selection ()
  "Selected-window resize delegates only when the policy is enabled."
  (should (fboundp 'ghostel--selected-window-resize))
  (let (adjusted)
    (cl-letf (((symbol-function 'ghostel--adjust-size)
               (lambda (window &optional force)
                 (setq adjusted (list window force)))))
      (let ((ghostel-resize-only-when-selected-window nil))
        (ghostel--selected-window-resize 'ghostel-window)
        (should-not adjusted))
      (let ((ghostel-resize-only-when-selected-window t))
        (ghostel--selected-window-resize 'ghostel-window)
        (should (equal adjusted '(ghostel-window nil)))))
    (setq adjusted nil)
    (let ((ghostel-resize-only-when-selected-window t)
          (ghostel--deferred-force-resize t))
      (cl-letf (((symbol-function 'ghostel--adjust-size)
                 (lambda (window &optional force)
                   (setq adjusted (list window force)))))
        (ghostel--selected-window-resize 'ghostel-window)
