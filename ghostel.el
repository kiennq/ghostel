...
materialized scrollback on every redraw."
  (let ((begin (or begin (point-min)))
    ...
    (save-excursion
      ...
      ;; Pass 2: file:line references (e.g. "foo/bar.el:42", "/tmp/bar.rs:10",
      ;;         "C:/Users/bar.rs:10").  The file-exists-p guard prevents
      ;;         false positives, so the regex is kept broad.
      ...
      (when (and ghostel--term (not ghostel--copy-mode-active))
        ...
        (unless (and (not ghostel--force-next-redraw)
          ...
          (when point-marker
            ...
            (when selected-win
              (set-window-point selected-win point-marker))
            (set-marker point-marker nil))))))))

(defun ghostel--anchor-selected-window-start (buffer rows)
  "Anchor BUFFER's selected window to the terminal viewport for ROWS."
  (when (and ghostel-cursor-follow
             (natnump rows)
             (> rows 0)
    ...
    (let ((viewport-start
      ...
      (set-window-start (selected-window) viewport-start))))
    ...
    (when (and size (buffer-live-p buffer))
      (with-current-buffer buffer
        (when ghostel--term
          ...
          (ghostel--anchor-selected-window-start buffer height))))
