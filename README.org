...
see.  It runs when the foreground shell is on a remote host
(=ghostel--remote-shell-p=, derived from the TRAMP =default-directory= ghostel
keeps in sync via OSC 7), and on Windows where ConPTY does not expose POSIX
termios state.  This gate avoids false positives from POSIX local raw-mode TUIs
like vim or less, and structural anchoring keeps shell-typed lines such as
=$ echo Password:= from triggering.  See =ghostel-debug-start= /
...
| =ghostel-redraw-only-when-selected-window= | =t= | Keep visible background terminal output pending until its window is selected.                                                                                            |
...
| =ghostel-password-prompt-regex=     | =comint-password-prompt-regexp=      | Cursor-row regex fallback (remote shells and Windows ConPTY).            |
...
The =pipeline/*/40x120= cases measure terminal updates through the production
redraw path, including viewport anchoring.  They cover one-cell changes,
identical row output, cursor movement, styled text, changing full frames,
mixed Unicode, and primary-screen scrolling.  Payload preparation and PTY
transport are excluded.

From the repository root in a graphical Emacs, include actual GUI redisplay:

#+begin_src emacs-lisp
(load-file (expand-file-name "ghostel-bench.el" "bench"))
(let ((ghostel-bench-force-gui-redisplay t))
  (ghostel-bench-run-one "pipeline/paired/40x120"))
#+end_src

Without =ghostel-bench-force-gui-redisplay=, the pipeline cases measure update
work without forcing the final Emacs redisplay.  Compare the same font, frame
size, native build options, and Lisp compilation mode.
...
src/NativeProcess.zig Background reader thread + native PTY backend selection
src/PosixPtyProcess.zig POSIX PTY: open master/replica, spawn the child
src/ConPtyProcess.zig Windows ConPTY: create pseudoconsole, spawn the child
