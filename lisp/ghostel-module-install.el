...
(defconst ghostel--minimum-module-version "0.56.0"
...
(defun ghostel--read-module-download-version ()
  ...
  (let ((version (read-string
    ...
    (unless (string= version "")
      (when (version< (ghostel--module-version-for-comparison version)
                      ghostel--minimum-module-version)
...
(defun ghostel--module-version-for-comparison (version)
  "Return VERSION with a release build suffix removed."
  (replace-regexp-in-string
   "\\`\\([0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\)\\.[[:xdigit:]]+\\'"
   "\\1" version))
...
triggers an interactive prompt."
  (let ((mod-ver (and (fboundp 'ghostel--module-version)
    ...
    (when (or (null mod-ver)
              (version< (ghostel--module-version-for-comparison mod-ver)
                        ghostel--minimum-module-version))
      (display-warning 'ghostel
