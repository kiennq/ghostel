...
`ghostel-module-compile' use a temporary Zig install prefix, then
copy the finished modules here."
  :type '(choice (const :tag "Use package directory" nil)
                 directory)
  :group 'ghostel)
...
(defun ghostel--runtime-support-artifacts (source-dir)
  "Return optional runtime artifact paths relative to SOURCE-DIR."
  (let* ((source-dir (file-name-as-directory
                      (expand-file-name source-dir)))
         (fixed '("ghostel-module.version"
                  "conpty.dll"
                  "x86/OpenConsole.exe"
                  "x64/OpenConsole.exe"
                  "arm64/OpenConsole.exe"))
         (pdbs (mapcar (lambda (path)
...
When DEST-DIR is non-nil, publish the artifacts there."
  (let* ((source-dir (ghostel--effective-module-dir source-dir))
    ...
    (dolist (relative (ghostel--runtime-support-artifacts source-dir))
      (let ((dest (expand-file-name relative dest-dir)))
        (make-directory (file-name-directory dest) t)
        (ghostel--replace-module-file
