...
(defconst ghostel--minimum-module-version "0.43.0"
...
(defun ghostel--copy-module-file (src dest)
  "Copy SRC to DEST unless both paths name the same file."
...
(defun ghostel--replace-loader-module-file (src dest)
  "Replace the stable loader at DEST with SRC.
The loader module can stay mapped by Emacs on Windows, so move the
old file aside before copying its replacement."
  (unless (ghostel--same-path-p src dest)
    (when (file-exists-p dest)
      (rename-file dest (ghostel--next-module-backup-path dest) t))
    (ghostel--copy-module-file src dest)))
...
When DEST-DIR is non-nil, publish the artifacts there."
  (let* ((source-dir (ghostel--effective-module-dir source-dir))
         ...
         (manifest-dest (ghostel--loader-metadata-path "ghostel-module.json" dest-dir))
         (support-files
          (seq-filter
           (lambda (relative)
             (file-regular-p (expand-file-name relative source-dir)))
           (delete-dups
            (append
             '("conpty.dll"
               "x86/OpenConsole.exe"
             ...
             (mapcar (lambda (path)
                       (file-relative-name path source-dir))
                     (directory-files-recursively source-dir "\\.pdb\\'")))))))
    ...
    (unless (file-directory-p dest-dir)
      (make-directory dest-dir t))
    (ghostel--replace-loader-module-file loader-src loader-dest)
    (ghostel--copy-module-file target-src target-dest)
    (ghostel--copy-module-file manifest-src manifest-dest)
    (dolist (relative support-files)
      (ghostel--copy-module-file
