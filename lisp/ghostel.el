...
(defvar-local ghostel--query-font-cache nil
  "Buffer-local cache for `query-font' results during native redraw.")
...
on the remote host."
  (let* ((state (ghostel--start-process-state))
         ...
         (proc (ghostel--spawn-pty (plist-get state :spawn-program)
