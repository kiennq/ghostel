...
recent typing and for the first frame after idle.  Hidden and copy-mode
output has no timer and remains pending until rendering resumes."
...
when the native renderer declines them during synchronized output or when
BUFFER has no visible render window."
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
...
(defun ghostel--pre-redisplay (_window)
  "Render pending terminal state before redisplay."
