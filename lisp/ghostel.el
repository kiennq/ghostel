...
(defun ghostel--window-adjust-process-window-size (process windows &optional force)
  "Resize the terminal to match the new Emacs window dimensions.
PROCESS is the shell process, WINDOWS is the list of windows.
When FORCE is non-nil, refresh the native terminal size and cell pixel
dimensions even when row and column counts are unchanged."
  (let* ((adjust-fn (default-value 'window-adjust-process-window-size-function))
    ...
    (when (and size selected-drives-resize (buffer-live-p buffer))
      (with-current-buffer buffer
        (when ghostel--term
          (cond
           ;; No change — skip entirely unless the cell dimensions changed.
           ((and (not force)
                 (eql height ghostel--term-rows)
