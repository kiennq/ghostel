...
export fn emacs_module_init(runtime: *c.struct_emacs_runtime) callconv(.c) c_int {
    ...
    env.bindFunction(
        ...
        1,
        &fnConptyIsAlive,
        "Return t if the Windows ConPTY child is alive.\n\n(conpty--is-alive TERM)",
    );
    env.bindFunction(
        "conpty--kill",
        1,
        1,
        &fnConptyKill,
        "Terminate the Windows ConPTY child.\n\n(conpty--kill TERM)",
    );
    env.bindFunction(
        "conpty--read-pending",
        1,
        1,
        &fnConptyReadPending,
        "Read pending Windows ConPTY output.\n\n(conpty--read-pending TERM)",
    );
    env.bindFunction(
        "conpty--resize",
        3,
        3,
        &fnConptyResize,
        "Resize the Windows ConPTY backend.\n\n(conpty--resize TERM ROWS COLS)",
    );
    env.bindFunction(
        "conpty--write",
        2,
        2,
        &fnConptyWrite,
        "Write raw bytes to the Windows ConPTY backend.\n\n(conpty--write TERM DATA)",
    );
